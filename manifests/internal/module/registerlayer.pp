...
define jboss::internal::module::registerlayer (
  ...
  if (!defined(Exec["jboss::module::layer::${layer}"])) {
    ...
    exec { "jboss::module::layer::${layer}":
      command => "touch ${layers_file} && awk -F'=' 'BEGIN {ins = 0} /^layers=/ { ins = ins + 1; print \$1\"=${layer},\"\$2 } END {if(ins == 0) print \"layers=${layer},base\"}' '${layers_file}' > '${tmp_file}' && mv '${tmp_file}' '${layers_file}'",
